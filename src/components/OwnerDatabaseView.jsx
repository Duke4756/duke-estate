...
const EMPTY_FILTERS = { search: '', intent: '', sourceRole: '', bedrooms: '', bathrooms: '', roomType: '', pet: '', system: '', stationId: '', transit: '', minPrice: '', maxPrice: '', minArea: '', maxArea: '', sort: 'newest' }
    ...
    <div className="mx-auto max-w-[1680px] px-5 py-6 lg:px-7">
      ...
      <section className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
        <div className="grid gap-px bg-slate-200 sm:grid-cols-2 xl:grid-cols-4">
          ...
          <button onClick={() => setShowAdvanced((value) => !value)} className="flex h-14 items-center justify-between bg-white px-4 text-left hover:bg-slate-50"><span><span className="block text-[10px] font-semibold text-slate-400">ตัวเลือกอื่น</span><span className="mt-0.5 block text-sm font-semibold text-slate-800">{showAdvanced ? 'ซ่อนตัวกรอง' : 'ตัวกรองเพิ่มเติม'}</span></span><span className="text-lg text-violet-600">{showAdvanced ? '−' : '+'}</span></button>
        </div>
        <div className="flex flex-wrap items-center justify-end gap-3 border-t border-slate-200 px-4 py-2">
        ...
        {showAdvanced && <div className="grid gap-3 border-t border-slate-200 bg-slate-50 p-4 sm:grid-cols-2 xl:grid-cols-4">
          <CompactSelect label="ระบบรถไฟฟ้า" value={filters.system} onChange={(v) => setFilters((f) => ({ ...f, system: v, stationId: '', transit: '' }))}><option value="">ทุกระบบ</option><option value="BTS">BTS</option><option value="MRT">MRT</option><option value="ARL">Airport Rail Link</option></CompactSelect>
          <CompactText label="พิมพ์ชื่อสถานี" value={filters.transit} onChange={(v) => setFilters((f) => ({ ...f, transit: v, stationId: '' }))} placeholder="เช่น อ่อนนุช, On Nut, พระราม 9" list="transit-station-options" /><datalist id="transit-station-options">{stations.map((station) => <option key={station.id} value={station.canonicalNameTh}>{station.systemCode} {station.canonicalNameEn}</option>)}</datalist>
          <CompactSelect label="ประเภทรายการ" value={filters.intent} onChange={(v) => setFilter('intent', v)}><option value="">เช่าและขาย</option><option value="rent">ให้เช่า</option><option value="sale">ขาย</option><option value="rent_and_sale">เช่าหรือขาย</option></CompactSelect>
          <CompactSelect label="ผู้ลงประกาศ" value={filters.sourceRole} onChange={(v) => setFilter('sourceRole', v)}><option value="">ทั้งหมด</option><option value="owner">เจ้าของ</option><option value="agent">เอเจนต์</option><option value="co_agent">Co-Agent</option></CompactSelect>
          <CompactSelect label="เรียงลำดับ" value={filters.sort} onChange={(v) => setFilter('sort', v)}><option value="newest">ล่าสุด</option><option value="price_asc">ราคาต่ำ → สูง</option><option value="price_desc">ราคาสูง → ต่ำ</option><option value="area_desc">พื้นที่มาก → น้อย</option></CompactSelect>
...
function FilterSelect({ label, value, onChange, options }) { return <label className="flex h-14 flex-col justify-center bg-white px-4"><span className="block text-[10px] font-semibold text-slate-400">{label}</span><select value={value} onChange={(e) => onChange(e.target.value)} className="mt-0.5 w-full cursor-pointer bg-transparent text-sm font-semibold text-slate-800 outline-none">{options.map(([v, text]) => <option key={v} value={v}>{text}</option>)}</select></label> }
...
function CompactText({ label, value, onChange, placeholder, list }) { return <label className="block"><span className="text-[10px] font-semibold text-slate-500">{label}</span><input type="search" list={list} value={value} onChange={(e) => onChange(e.target.value)} placeholder={placeholder} className="mt-1 h-10 w-full rounded-lg border border-slate-300 bg-white px-3 text-sm outline-none placeholder:text-slate-400 focus:border-violet-400" /></label> }
...
function projectNames(row) { const candidates = [row.project_name_en, row.project_name_th, row.project_name_canonical, row.project_name_raw].filter(isDisplayableProjectName); return [...new Set(candidates)].slice(0, 2).concat(candidates.length ? [] : ['ไม่ระบุชื่อโครงการ']) }
function isDisplayableProjectName(value) { const name = String(value || '').trim(); return name.length >= 3 && name.length <= 90 && !/#|ดู(?:เพิ่มเติม|น้อยลง)|^(?:สถานที่ใกล้|สถานที่สำคัญ|สิ่งอำนวยความสะดวก|จุดเด่น|corner unit|duplex|loft|penthouse|studio|อสังหาริมทรัพย์ให้เช่า)$/iu.test(name) }
